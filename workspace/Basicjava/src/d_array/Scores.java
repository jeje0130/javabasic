...
public class Scores {
	...
	public static void main(String[] args) {
		...
		int[][] score = new int[names.length][subjects.length];      //int[학생수][과목수]
		
		for(int i = 0; i < score.length; i++){                      //랜덤으로 점수 출력
			for(int j = 0; j < score[i].length; j++){
				score[i][j] = (int)(Math.random() * 101);
			}
		}
		
		int[] nameSum = new int[score.length];                      //개인별 합계
		double[] nameAvg = new double[score.length];	            //개인별 평균
		...
		int[] subSum = new int[subjects.length];                   // 과목별 합계
		double[] subAvg = new double[subjects.length];             // 과목별 평균
...
//		}
		...
		//출력
		for(int i = 0; i < subjects.length; i++){   //과목명 출력
		...
		for(int i = 0; i < score.length; i++){
			...
			System.out.println(nameSum[i] + "\t" + nameAvg[i] + "\t" + rank[i]);   // 개인 합계, 평균, 석차 출력 
		}
